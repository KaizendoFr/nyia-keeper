...
- **Prefer this skill** (`/nyia-make-a-plan`) — it creates structured, trackable plans in
...
(its decision log is `{number}-{slug}/decisions.md`, appended in the format of section E2; reviews
land in `{number}-{slug}/reviews/`). Never create the legacy flat `{number}-{slug}.md` — the
host resolvers, the generated inventory and the review skills key on the directory shape.
...
`Draft Ready Active Blocked Review Done Dropped`. The `todo.md` inventory is generated by the
host from these fields at launch and after the session — never hand-edit `todo.md`.

Optionally, a plan MAY carry a free-text `Roadmap:` line (e.g. `Roadmap: mvp`, `Roadmap: RC1`) to
place it on a roadmap axis the user defines — the `/nyia-plan-status` view groups by it. Omit it if the
...
2. Each phase becomes a subplan directory: `{N}a-{phase-slug}/plan.md`, `{N}b-.../plan.md`, etc.
   Downstream commands reference subplans by their full number (e.g., `/nyia-plan-review plan 230a`)
...
(append to `<N>-<slug>/decisions.md` with a blank line before the entry; one line per field; no secrets;
see docs/PLAN_FILE_CONTRACT.md. The host CLI is not available inside the container.)
```
...
- `todo.md` is a generated, read-only inventory (written by the host at launch/exit) built from each plan's
