...
- Accept plan numbers or file paths: `/nyia-run-plans 212 213 206`
...
3. Confirm each plan has `Status: Ready` in its `plans/NNN-slug/plan.md` (`.nyiakeeper/todo.md` is the generated inventory; if stale, read the `Status:` lines directly)
...
- Maintain each plan's `Status:` in its `plan.md` as you go — `Active` when it starts, `Done` when it
  completes, `Blocked` on a blocker. The `todo.md` inventory is GENERATED by the host from these fields; never hand-edit
  `todo.md`.
