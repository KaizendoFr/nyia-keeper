...
export PATH="$HOME/.local/bin:$PATH"
```
See [Windows (WSL2) Setup Guide](WSL2_SETUP.md). (WSL1 is not supported.)
...
[Plan tracking](CLI_REFERENCE.md#plan-tracking). `nyia` itself never runs inside the container: the skills
work with the plan files directly ([the file contract](PLAN_FILE_CONTRACT.md)).
