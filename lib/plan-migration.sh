...
classify_legacy_file() {
    ...
    case "$b" in
        plan-review-*|code-review-*|pair-review-*)
            ...
            num="$(printf '%s' "$b" | sed -nE 's/.*plan-([0-9]+[a-z]?)[-.].*/\1/p')"
...
#     BLOCK <number>        (reason)         — a number with >1 body: fail-closed, its files stay flat
#     SKIP  <basename>                       — unclassified / orphan review / a blocked number's file
#     UNSAFE <count>                         — basenames with a tab/newline: never moved, marker withheld
# The executor consumes this plan; the union invariant is: every source file is a MOVE src, a SKIP, or UNSAFE.
plan_migration() {
    local dir="${1:-$NYIA_PLANS_DIR}" f b cat key num unsafe=0
...
backfill_migrated_statuses() {
    ...
    for p in "$dir"/[0-9]*-*/plan.md; do
        ...
        case "$res" in
            wrote*) IFS=$'\t' read -r _ st rel <<<"$res"; printf '  %s → %s\n' "${p#"$dir"/}" "$st" ;;
        esac
    done
    return 0
}
...
execute_migration() {
    ...
    mapfile -t plan < <(plan_migration "$dir")
    ...
    local unsafe_total=0 skipped_legacy=0 _bdir _bnum
    declare -A body_dirs                                   # number → its plan dirs (for the collisions log)
    for l in "${plan[@]}"; do
        IFS=$'\t' read -r action src dest <<<"$l"
        case "$action" in
            MOVE)
            ...
            SKIP)
                read -r c _ <<<"$(classify_legacy_file "$src")"   # classify output is space-delimited
                case "$c" in
                    skip) ;;                                      # foreign file → doesn't block the marker
                    *)    skipped_legacy=$((skipped_legacy + 1)); remnant=1 ;;  # orphan review/state → an unrouted artifact
                esac ;;
        esac
    done
    ...
    if [[ ${#note_report[@]} -gt 0 ]]; then
        local logf="$dir/collisions.md" n
        {
    ...
    # Partial: genuine remnants stayed flat (orphan reviews with no body, or unsafe names). No data lost.
    {
        ...
        [[ "$skipped_legacy" -gt 0 ]] && printf '  - %s unrouted legacy artifact(s) (e.g. a review whose plan number has no body).\n' "$skipped_legacy"
        [[ "$unsafe_total" -gt 0 ]] 2>/dev/null && printf '  - %s file(s) with unsafe names (tab/newline).\n' "$unsafe_total"
    } >&2
    return 3
}
